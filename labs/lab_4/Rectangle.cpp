...
bool Rectangle::add_point(const Point2D& p){
    if (is_point_within(p)){
        if (std::find(m_points_contained.begin(), m_points_contained.end(), p) 
              == m_points_contained.end()){
...
std::vector<Point2D> points_in_both(const Rectangle& r1, const Rectangle& r2) {
 ...
 // create and return a vector that contains the points inside of both rectangles
    std::vector<Point2D> in_both;

    for (unsigned i = 0; i < r1_points.size(); i++){
        if (std::find(r2_points.begin(), r2_points.end(), r1_points[i]) 
              != r2_points.end()){
            in_both.push_back(r1_points[i]);
        }
    }
...
void print_rectangle(const Rectangle& r1) {
    ...
    std::cout << "Rectangle: " 
              ...
              << "--"
              << r1.upper_right_corner().x() << "," << r1.lower_left_corner().y() 
              << std::endl;
