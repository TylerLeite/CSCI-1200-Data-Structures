...
vector<vector<Point> > recursive_find_paths(int x, int y, const vector<vector<bool> >& blocked_grid){
    ...
    if (x == 0 && y == 0){
        ...
        if (x > 0 && !blocked_grid[x-1][y]){
            ...
            vector<vector<Point> > tmp = recursive_find_paths(x-1, y, blocked_grid);
        ...
        if (y > 0 && !blocked_grid[x][y-1]){
            any_possible_moves = true;
            vector<vector<Point> > tmp = recursive_find_paths(x, y-1, blocked_grid);
            for (int i = 0; i < tmp.size(); i++){
                out.push_back(tmp[i]);
            }
        }
        ...
        if (!any_possible_moves){
            possible_moves.push_back(Point(-1, -1)); //dead end
        }
    }
}

bool path_contains(vector<Point> path, int x, int y){
...
vector<vector<Point> > find_paths(int x0, int y0, const vector<vector<bool> >& blocked_grid){
    vector<vector<Point> > paths = recursive_find_paths(x0, y0, blocked_grid);
    
    //remove dead ends
    for (vector<vector<Point> >::iterator it = paths.begin(); it != paths.end(); it++){
        for (int i = 0; i < it->size(); i++){
            if (path_contains(*it, -1, -1)){
                it = paths.erase(it);
                break;
            }
        }
...
void print_paths(const vector<vector<Point> >& paths, const vector<vector<bool> >& blocked_grid, int start_x, int start_y){
    if (paths.size() == 0){cout << "didnotwork" << endl;}
        ...
        for (int j = 0; j < blocked_grid[0].size(); j++){
            for (int i = 0; i < blocked_grid.size(); i++){
                if (i == start_x && j == start_y){
                    cout << " S";
                } else if (blocked_grid[i][j]) {
                    cout << " X";
                } else {
                    if (path_contains(paths[n], i, j)){
                        cout << " P";
                    } else {
                        cout << " .";
                    }
...
int main(int argc, char* argv[]) {
  ...
  // Start here with your code...
    ...
    all_paths = recursive_find_paths(start_x, start_y, blocked_grid);
    print_paths(all_paths, blocked_grid, start_x, start_y);  
