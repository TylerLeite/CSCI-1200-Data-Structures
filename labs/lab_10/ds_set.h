...
public:
  ...
  TreeNode<T>* left(){return ptr_->left;}
  TreeNode<T>* right(){return ptr_->right;}
...
public:
  ...
  int erase(T const& key_value) { return erase(key_value, root_); }
    ...
    //My code
      iterator find_iterative(const T& key_value){
        std::list<iterator> untested;
        untested.push_back(root_);
        while (untested.size() != 0){
            iterator current = untested.front();
            
            if (*current == key_value) return current;
            if (current.left()  != NULL && *current > key_value) untested.push_back(current.left());
            if (current.right() != NULL && *current < key_value) untested.push_back(current.right());
...
private:
  ...
  TreeNode<T>* copy_tree(TreeNode<T>* old_root) {
        if (old_root == NULL){
            ...
            TreeNode<T>* new_node = new TreeNode<T>(old_root->value);
        
            if (old_root->left != NULL){
                new_node->left = copy_from(old_root->left);
            } else {
                new_node->left = NULL;
            }

            if (old_root->right != NULL){
                new_node->right = copy_from(old_root->right);
            } else {
                new_node->right = NULL;
            }
