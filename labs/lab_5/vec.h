...
template <class T>
void Vec<T>::print(std::ostream& out) {
    out << "m_size:  " << m_size << std::endl
...
//loop through the vector adding all non-T elements to a new vector. set the old
// vector to be equal to the new vector. runs in O(n + (n-e)) = O(ne)
template <typename T>
unsigned remove_matching_elements(Vec<T>& vec, const T& element){
    unsigned remv = 0;
/*
    Vec<T> newVec;
