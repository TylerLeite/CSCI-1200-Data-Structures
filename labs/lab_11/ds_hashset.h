...
public:
  ...
  std::pair< iterator, bool > insert(KeyType const& key) {
    ...
    // Implement this function for Lab 13, Checkpoint 1
        unsigned hash_value = m_hash(key);
        unsigned index = hash_value % m_table.size();
        
        hash_list_itr out = std::find(m_table[index].begin(), m_table[index].end(), key);

        bool success = false;
        if (out == m_table[index].end()){
            m_table[index].push_back(key);
            out = m_table[index].end();
            out--;
            success = true;
            m_size++;
        }

        iterator real_out = iterator(this, index, out);

        return std::pair<iterator, bool>(real_out, success);
  }
  ...
  iterator begin() {
    return iterator(this, 0, m_table[0].begin());
  }
  ...
  void resize_table(unsigned int new_size) {
    ...
    //
      ...
      while (it != end()){
        KeyType key = *it;
        unsigned hash_value = m_hash(key);
        unsigned index = hash_value % new_size;
