...
/// FILL IN THIS FUNCTION
    ...
    if (school_it == schools.end()){
            ...
            std::cerr << "WARNING: could not insert student into school preference list, no school named " 
        ...
        for (; student_it != pref_list->end(); student_it++){
            if (*student_it == new_student_name){
                std::cerr << "WARNING: could not add " << new_student_name
                          << " into school preference list, this student is already in the list"
                          << std::endl;
            }
      ...
      //insert new student in the proper location
        if (temp == pref_list->end()){
            std::cerr << "WARNING: could not insert new student into school preference list before " 
                      << insert_before_student_name << ", this student is not in the list" << std::endl;
            //exit(1);
            return;
        } else {
...
/// FILL IN THIS FUNCTION
    ...
    if (student_it == students.end()){
        ...
        std::cerr << "WARNING: could not remove school from student preference list, no student named " 
    ...
    std::cerr << "WARNING: could not remove " << school_name
              << "from student preference list, that school is not in the list" 
              << std::endl;
  ...
  while(1) {
	...
	if (student_itr->GetName() == student) {
	  ...
	  if (!tentative_acceptance) {
        ...
        if (!student_exists){
            std::cerr << "WARNING: could not make offer to student " << student 
...
/// FILL IN THIS FUNCTION
    ...
    std::cerr << "WARNING: could not print student preference list for "
              << school_name << ", that school does not exist" << std::endl;
    //exit(1);
}
...
/// FILL IN THIS FUNCTION
    ...
    std::cerr << "WARNING: could not print school preference list for "
              << student_name << ", that student does not exist" << std::endl;
    //exit(1);
}
