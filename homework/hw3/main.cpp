...
void StudentTests() {
  ...
  //
    ...
    //jaggedarrays of ints (100 bins)
    JaggedArray<int>* intJa = new JaggedArray<int>(100);
    srand(time(NULL));

    std::cout << "ADDING 1000 RANDOM ELEMENTS TO RANDOM BINS" << std::endl;
    for (int i = 0; i < 1000; i++){
        int bin = rand()%100;
        int ele = rand()%19-9;

      //pack and unpack 1000 times for good measure
        intJa->pack();
        intJa->unpack();
        
        intJa->addElement(bin, ele);
    }

    std::cout << "RANDOMLY REMOVING A BUNCH OF ELEMENTS ONE AT A TIME" 
              << std::endl;
    for (int i = 0; i < 100; i++){
        while (intJa->numElementsInBin(i) > 1){
            intJa->removeElement(i, 0);
        }
    }
    ...
    intJa->pack();
    intJa->addElement(0, 1);
    intJa->unpack();
    intJa->addElement(0, 1);
    intJa->removeElement(9001, 12);
    intJa->removeElement(0, 2);
    intJa->pack();
    intJa->removeElement(0, 1);
    intJa->unpack();
    intJa->removeElement(0, 1);
