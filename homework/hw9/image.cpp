...
template<>
Image<Offset>::Image(int width, int height) :
    width(width), height(height), data(new Offset[width*height])
{
    SetAllPixels(Offset()); //0
}

template<>
Image<bool>::Image(int width, int height) :
    width(width), height(height), data(new bool[width*height])
{
    SetAllPixels(false); //white
}
